...
async function renderApp(
  ...
  return routerLocationAsyncLocalStorage.run({pathname, search}, async () => {
    ...
    const rscAppStream = createRscAppStream(
      acceptHeader === `text/x-component-remote` ? <App /> : <Bootstrap />,
      {reactClientManifest, mainCssHref: cssManifest[`main.css`]!, formState},
    );

    if (acceptHeader?.startsWith(`text/x-component`)) {
