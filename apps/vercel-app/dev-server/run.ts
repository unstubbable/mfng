...
// @ts-ignore
import handler from '../dist/dev-server-handler.js';
import type {Handler} from '../src/edge-function-handler/index.js';
import {transformHeaders} from './transform-headers.js';

const app = new Hono();

app.use(`/client/*`, serveStatic({root: `.vercel/output/static`}));
...
const server = serve({fetch: app.fetch, port: 3001}, ({address, port}) => {
